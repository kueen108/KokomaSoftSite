---
title: "오늘의 AI 글: Building Jarvis #1: Getting a Local LLM to Read"
description: "Medium 글 'Building Jarvis #1: Getting a Local LLM to Read a PDF It Cannot Fit in Context'를 바탕으로, AI 에이전트를 일회성 프롬프트가 아니라 반복 가능한 업무 흐름, 컨텍스트 관리, 검증 규칙으로 다루는 법을 정리한다."
pubDate: "2026-09-24"
sourceTitle: "Building Jarvis #1: Getting a Local LLM to Read a PDF It Cannot Fit in Context"
sourceUrl: "https://medium.com/@aaryacodes/building-jarvis-1-getting-a-local-llm-to-read-a-pdf-it-cannot-fit-in-context-227d10733624"
sourceAuthor: "Aarya Bookseller"
...
오늘 고른 Medium 글은 그 지점을 건드린다. 원문은 "Building Jarvis #1: Getting a Local LLM to Read a PDF It Cannot Fit in Context"라는 제목에서 보이듯, AI 에이전트를 단발성 대화가 아니라 반복 가능한 작업 흐름으로 다루는 관점을 제시한다. 이 주제는 단순한 제품 마케팅 비평이 아니다. Claude Code, Codex, Cursor, Gemini CLI 같은 코딩 에이전트도 결국 같은 시험대에 오른다. 처음에는 신기함이 사용을 만든다. 그러나 시간이 지나면 남는 것은 팀의 습관, 리뷰 기준, 비용 구조, 실패했을 때의 복구 가능성이다.
...
- I use AI almost every day. I give it class notes, papers, presentations, and long documents and ask questions about them without really&#x2026; 이 요지는 프롬프트 실력보다 업무를 구조화하고 결과를 검증하는 운영 습관이 더 오래 간다는 점을 보여준다. 새 기능을 빠르게 만드는 장면은 인상적이지만, 팀이 매일 맡겨도 되는 작업과 사람이 반드시 붙어야 하는 작업을 구분하지 못하면 속도는 곧 리스크가 된다.
