...
function recentSourceTitles() {
  return new Set(
    readdirSync(postDir)
      .filter((name) => /^\d{4}-\d{2}-\d{2}-.*\.md$/.test(name))
      .sort()
      .slice(-20)
      .map((name) => readFileSync(join(postDir, name), 'utf8').match(/^sourceTitle:\s*["']?(.+?)["']?$/m)?.[1])
      .filter(Boolean),
  );
}
...
function score(candidate, usedTitles) {
  if (usedTitles.has(candidate.title)) return -1000;
...
function koreanPost({ date, slug, candidate, accessNote }) {
  ...
  const sourcePhrase = `원문은 "${candidate.title}"라는 제목에서 보이듯, AI 도구를 데모나 유행어가 아니라 실제 사용 맥락에서 보자는 문제의식을 던진다.`;
  ...
  return `---
title: "오늘의 AI 글: AI 도구의 가치는 데모가 아니라 채택에서 결정된다"
description: "Medium 글 '${safeTitle}'를 바탕으로, AI 도구와 코딩 에이전트의 가치를 기능 시연이 아니라 실제 업무 채택, 반복 사용, 검증 가능한 운영 기준으로 판단해야 하는 이유를 정리한다."
pubDate: "${date}"
...
- ${summary} 이 요지는 코딩 에이전트에도 그대로 적용된다. 새 기능을 빠르게 만드는 장면은 인상적이지만, 팀이 매일 맡겨도 되는 작업과 사람이 반드시 붙어야 하는 작업을 구분하지 못하면 속도는 곧 리스크가 된다.
...
const postPath = join(postDir, slug);
